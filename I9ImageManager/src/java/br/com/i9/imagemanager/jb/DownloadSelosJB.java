 ...
 * @author geoleite
 */
public class DownloadSelosJB extends SystemBase {

    private Pro_produtoT pro_produtoT = new Pro_produtoT();

    private byte[] readFile() {
        File file = new File(PASTA_BASE.concat(getSession().getId().concat("_selos.zip")));
        try {
    ...
    public void pageLoad() {
        try {
            ...
            getRequest().setAttribute(EasyDownloadJB.CONTENT_TYPE, "arquivo/zip");
            getRequest().setAttribute(EasyDownloadJB.FILE_NAME, pro_produtoT.getPro_tx_nome().concat("_selos.zip"));
